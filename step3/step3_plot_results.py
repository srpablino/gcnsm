...
#                             fig1.suptitle(str("NegSample: {}x Optimizer: {} Architecture: {} Loss: {} lr: {} splits: {} Loss margin: {}").format(sampling,op,a,lf,enot,s,loss_parameters[lp]),fontsize=18)
                            ...
                            axs[0].yaxis.grid(True)
                            axs[0].tick_params(labelsize=16)
                            axs[0].plot(range(0,len(df_results0)), acc, marker="o", c=colors[lp],  linestyle='--', label="Max accuracy: "+ str("{:.5f}".format(max_acc)))
                            axs[0].set_title("Test accuracy",fontsize=16)
                            leg = axs[0].legend(loc='best', ncol=1, shadow=True, fancybox=True,fontsize=16)
...
#     lf = loss_functions[0]
    ...
    #                         break
                        ...
                        fig1, axs = plt.subplots(1, 1, figsize=(35, 10), facecolor='w', edgecolor='k')
                        enot = str("{:e}".format(lr))
                        enot = enot.split("e")[0].rstrip('0').rstrip(".")+"e"+enot.split("e")[1]
                        file_name = str("loss_parameters:{}|batch_splits:{}|lr:{}|epochs_run:{}_{}".format(loss_parameters[lp],s,enot,eri,ere))
...
#                             acc2 = [log["acc2"] for log in logs]
        ...
        #                     axs[2].set_ylim([0,1])
                            ...
                            axs.set_yticks(np.arange(0.0, 1.05, 0.05))
                            axs.yaxis.grid(True)
                            axs.tick_params(labelsize=18)
                            axs.tick_params(labelsize=18)

#                             axs[0].plot(range(0,len(logs)), loss, marker="o", c=colors[df_results0[i][0]],  linestyle='--', label="Run="+str(df_results0[i][0]+1))
                            axs.plot(range(0,len(logs)), acc, marker="o", c=colors[df_results0[i][0]],  linestyle='--', label="Run="+str(df_results0[i][0]+1)+" max_acc= "+str("{:.5f}".format(max_acc)))
        #                     axs[2].plot(range(0,len(df_results0)), acc2, marker="o", c=colors[lp],  linestyle='--', label=" Loss parameters: margin="+loss_parameters[lp])

#                             axs[0].set_title("Train loss (x1000)",fontsize=16)
                            axs.set_title("Test accuracy",fontsize=18)
        #                     axs[2].set_title("Test accuracy Nearest neighbor")

#                             leg = axs[0].legend(loc='best', ncol=1, shadow=True, fancybox=True,fontsize=16)
                            leg = axs.legend(loc='best', ncol=1, shadow=True, fancybox=True,fontsize=18)
        #                     leg = axs[2].legend(loc='best', ncol=1, shadow=True, fancybox=True)
...
#                         axs.set_title(str("Test Fscore AVG: {:.5f} - Var: {:.5f} - SD: {:.5f}".format(max_fscore_avg,var_max_fscore,std_max_fscore)),fontsize=18)
                        fig1.suptitle(str("Cross Validation Accuracy AVG: {:.5f} - Var: {:.5f} - SD: {:.5f}".format(max_acc_avg,var_max_acc,std_max_acc)),fontsize=18)
...
#                                 axs[1].set_yticks(np.arange(0.5, 1.05, 0.05))
                                ...
                                axs.yaxis.grid(True)
                                axs.tick_params(labelsize=18)
                                axs.plot(range(0,len(fscore)), fscore, marker="o", c=colors[run],  linestyle='--', label=str("Run:{:.0f}, Max Fscore: {:.5f}, with Recall: {:.5f} and Precision: {:.5f}".format(df_results0[i][0]+1,max_fscore[0],max_fscore[1],max_fscore[2])))
                        except:
                            continue
...
#                         axs[0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=1, shadow=True, fancybox=True,fontsize=16)
                        axs.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=1, shadow=True, fancybox=True,fontsize=16)
#                         axs[0].set_title("Train Loss (x1000)",fontsize=16)
                        axs.set_title(str("Test Fscore AVG: {:.5f} - Var: {:.5f} - SD: {:.5f} ; Test Recall AVG: {:.5f} - Var: {:.5f} - SD: {:.5f} ; Test Precision AVG: {:.5f} - Var: {:.5f} - SD: {:.5f}".format(max_fscore_avg,var_max_fscore,std_max_fscore,max_recall_avg,var_max_recall,std_max_recall,max_prec_avg,var_max_prec,std_max_prec)),fontsize=18)
